...
public class MaxNumber {
    public static void main(String[] args) {
        System.out.println(Arrays.toString(maxNumber(new int[]{8, 9}, new int[]{5,4,6}, 2)));
    }
    /**
     * 本质上对两个数组进行排序，考虑使用归并？
     * 使用归并来尝试下
    ...
    public static int[] maxNumber(int[] nums1, int[] nums2, int k) {
        ...
        while(right < nums2.length) {
            if (stack2.isEmpty()) {
                ...
                if (stack2.peekLast() < nums2[right]) {
                    stack2.pollLast();
                }
                stack2.offerLast(nums2[right]);
            }
        ...
        int[] arr = new int[k];
        for(int i = 0; i < k; i++) {
            Integer num1 = stack1.peekFirst();
            Integer num2 = stack2.peekFirst();
            if (num1 == null) {
                arr[i] = stack2.pollFirst();
            } else if (num2 == null) {
                arr[i] = stack1.pollFirst();
            } else {
                arr[i] = num1 > num2 ? stack1.pollFirst() : stack2.pollFirst();
            }
        }
        return arr;
    }
